...
const ImageSelection = ({ handleSelectImage }) => {
  ...
  const handleApply = () => {
    handleSelectImage(images[selectedImage]);
  };
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <ImageSelection handleSelectImage={handleSelectImage} />
      </DialogContent>
    </Dialog>
  );
};
...
const PathwayBuilder = ({ pathwayToEdit = false }) => {
  ...
  const handleCheckboxChange = (e) => {
    setPathway({ ...pathway, [e.target.name]: e.target.checked });
  };
  ...
  return (
    <div className="mt-4 flex justify-center">
      ...
      >
        ...
        {emojiPickerOpen && <div>Select Emojis Component</div>}
